    ...
    */
    ...
    $sysAlarmsHist = Alarms::where('alarms.system_id', $sid)
      ...
      $sysAlarmsHist = Alarms::where('alarms.system_id', $sid)
        ...
        ->where('active',0)
        ->count();
    } else {$sysAlarmsHist = Alarms::where('alarms.system_id', $sid)
         ->join('devices', 'devices.id', '=', 'alarms.device_id')
         ->where('devices.system_id',$sid)
         ->where('devices.device_io',$filter)
         ->where('device_id','<>',0)
    ...
    $reccountHist=$sysAlarmsHist;
     ...
     if ($filter=="All")  {
      ...
      $sysAlarmsActive = Alarms::where('alarms.system_id', $sid)
        ...
        ->take($take+1)
        ->get();
    } else {$sysAlarmsActive = Alarms::where('alarms.system_id', $sid)
         ->join('devices', 'devices.id', '=', 'alarms.device_id')
         ->where('devices.system_id',$sid)
         ->where('devices.device_io',$filter)
         ->where('device_id','<>',0)
  ...
  */
  ...
  public function dashboard()
  {
    ...
    foreach($customer_buildings as $building) {
      ...
      $dashboard_alarm = 0;
      foreach($systems as $system) {
        if($system->alarm_severity>$dashboard_alarm) $dashboard_alarm = $system->alarm_severity;
      }
...
*/
  ...
  public function building($building_id)
  {
    ...
    $building_info = '';
    if($building->address1 !== '') {$building_info .= $building->address1 . '<br>';}
    if($building->address2 !== '') {$building_info .= $building->address2 . '<br>';}
    if($building->city !== '') {$building_info .= $building->city;}
    if($building->city !== '' && $building->state !== '') {$building_info .= $building->state;}
    if($building->zip > 0) {$building_info .= $building->zip;}
