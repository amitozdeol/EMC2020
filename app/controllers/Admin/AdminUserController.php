...
class AdminUserController extends BaseController {
  ...
  public static function validate_password($password1 = NULL, $password2 = NULL)
  {
    ...
    try{
      if($password1 === NULL) {$password1 = Input::get('password');}
      if($password2 === NULL) {$password2 = Input::get('re-password');}
    } catch (Exception $e) {
      return false;
    }
  ...
  private function validate_role($role = NULL, $customer_id = NULL)
  {
    ...
    try{
      if($role === NULL)        {$role        = intval(Input::get('role'));}
      if($customer_id === NULL) {$customer_id = intval(Input::get('customer_id'));}
    } catch (Exception $e) {
      return false;
    }
